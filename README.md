...
```bash
# 1. Install pcc-node (the operator daemon — has the Hamilton adapter compiled in)
...
The `HamiltonAdapter` runtime class itself lives in [`@pcc/kernel`](https://github.com/LamaSu/physical-capability-cloud/tree/master/packages/kernel/src/adapters/hamilton-adapter.ts) — this repo is the operator-facing config helpers and the published, stable shape for that config.
...
**`unknown adapter type 'hamilton'`** — the kernel runtime is older than this adapter. Update `pcc-node` (or the upstream `@pcc/kernel`) to a version that includes the Hamilton adapter (≥0.1.0 of `@pcc/kernel` post-2026-05-06).
...
- [`physical-capability-cloud`](https://github.com/LamaSu/physical-capability-cloud) — main PCC monorepo (gateway, kernel, contracts, adapter implementations)
- [`agent-package.json`](https://capability.network/agent-package.json) — 230+ tools for any LLM, includes the role-aware bootstrap prompt
- [Hamilton Developer Portal](https://developer.hamiltoncompany.com/products/prep/api/openapi) — Hamilton's official API docs
...
For larger contributions (new Hamilton product line — STAR / STARlet / Vantage — or a different vendor's liquid handler), open an issue first to discuss the design.
